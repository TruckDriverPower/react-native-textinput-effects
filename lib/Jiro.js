...
import { Animated, Easing, Text, TextInput, TouchableWithoutFeedback, View, StyleSheet } from "react-native";
...
export default class Jiro extends BaseInput {
  ...
  _toggle(isActive) {
    ...
    if (isActive) {
      Animated.sequence([borderPositionAnimation, Animated.parallel([labelPositionAnimation, borderHeightAnimation])]).start();
    } else {
      Animated.sequence([borderHeightAnimation, Animated.parallel([borderPositionAnimation, labelPositionAnimation])]).start();
    }
  }

  render() {
    const { label, style: containerStyle, inputStyle, labelStyle, borderColor, height: inputHeight } = this.props;
    const { width, borderPositionAnim, borderHeightAnim, labelPositionAnim, value } = this.state;
